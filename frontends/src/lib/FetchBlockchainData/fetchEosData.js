...
const fetchEosTokenData = async (balanceArr, thisObject) => {
  const tokenBalanceArr = [...balanceArr];
  let eosTokenApiUrl = '';
  for (let i = 0; i < balanceArr.length; i++) {
    ...
    res.data.tokens.forEach((el) => {
      tokenBalanceArr[i].tokens.push({
        symbol: el.currency,
        totalTokenValue: el.usd_value,
        balance: el.amount
      });
...
const fetchEosData = async (address, thisObject) => {
  ...
  for (let i = 0; i < address.length; i++) {
    res = await axios.post(
      `${thisObject.state.eosBalanceApiUrl}`,
      JSON.stringify({ account_name: address[i] })
    );
    balanceArr.push({
  ...
  await thisObject.setState({
    eosBalanceArr: tokenBalanceArr,
    eosTotalBalance: totalEosBalance * thisObject.state.eosPrice,
    eosTotalTokenBalance: totalTokenBalance
  });
};
